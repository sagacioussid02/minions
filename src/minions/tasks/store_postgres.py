    ...
    def save(self, task: Task) -> Task:
        ...
        with connect() as conn, conn.cursor() as cur:
            cur.execute(
                ...
                """,
                (
                    str(task.id), str(task.decision_id), task.project, task.sprint_number,
                    task.category, task.title, task.description, task.acceptance_criteria,
                    task.owner_role, task.owner_agent_id, task.owner_display_name,
                    task.estimated_effort, task.status, task.pr_url, task.pr_number,
...
def _row_to_task(row: tuple) -> Task:
    (
        id_, decision_id, project, sprint_number, category, title,
        description, acceptance_criteria, owner_role, owner_agent_id,
        owner_display_name, estimated_effort, status, pr_url, pr_number,
        created_at, updated_at, completed_at, payload,
    ) = row
