...
def bootstrap_tenant(
    ...
    from minions.config.portfolio import load_portfolio_config
    from minions.crews.engineer_runs_store_factory import make_engineer_runs_store
    from minions.db.connection import connect, tenant_scope
    from minions.dossiers.store_factory import make_dossier_store
    ...
    with tenant_scope(tenant_id):
        with connect() as conn, conn.cursor() as cur:
            cur.execute(
                "SELECT project, manifest_json->>'name' FROM tenant_projects WHERE tenant_id = %s",
                (tenant_id,),
            )
            rows = cur.fetchall()
        projects = [row[0] for row in rows]
        # activity_log.project is the manifest's display name everywhere else
        # (Decisions, EngineerRuns, scrum) — not the URL slug tenant_projects
        # keys on. Fall back to the slug only if the manifest has no name yet.
        names_by_slug = {row[0]: (row[1] or row[0]) for row in rows}
